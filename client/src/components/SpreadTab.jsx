...
export default function SpreadTab() {
  ...
  const getAllList = async () => {
    try {
      await axios
        .get('https://betting-app-tl.onrender.com/api/spread')
//        .get('http://localhost:5000/api/spread')
        .then((response) => {
              ...
              <Table sx={{minWidth: 320}} aria-label='betting app table'>
                ...
                <TableBody>
                  <TableRow key={betevo.key+"1"}>
                    ...
                    <TableCell className={classes.tableCell}>{betevo.betevo_visitor_spread_odd+" "}{betevo.betevo_visitor_spread_stand}</TableCell>
                    <TableCell className={classes.tableCell}>{betevo.sport_visitor_spread_odd+" "}{betevo.sport_visitor_spread_stand}</TableCell>
                  </TableRow>
                  <TableRow key={betevo.key+"2"}>
                    <TableCell className={classes.tableCell}>{betevo.home_team}</TableCell>
                    <TableCell className={classes.tableCell}>{betevo.betevo_home_spread_odd+" "}{betevo.betevo_home_spread_stand}</TableCell>
                    <TableCell className={classes.tableCell}>{betevo.sport_home_spread_odd+" "}{betevo.sport_home_spread_stand}</TableCell>
                  </TableRow>
                </TableBody>
              </Table>
            </CardContent>
