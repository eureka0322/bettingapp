...
export default function TotalTab() {
  ...
  const getAllList = async () => {
    try {
      await axios
      .get('https://betting-app-tl.onrender.com/api/total')
//      .get('http://localhost:5000/api/total')
      .then((response) => {
              ...
              <Table sx={{minWidth: 320}} aria-label='betting app table'>
                ...
                <TableBody>
                  <TableRow key={betevo.key+"1"}>
                    ...
                    <TableCell className={classes.tableCell}>{betevo.betevo_visitor_total_odd+" "}{betevo.betevo_visitor_total_stand}</TableCell>
                    <TableCell className={classes.tableCell}>{betevo.sport_visitor_total_odd+" "}{betevo.sport_visitor_total_stand}</TableCell>
                  </TableRow>
                  <TableRow key={betevo.key+"2"}>
                    <TableCell className={classes.tableCell}>{betevo.home_team}</TableCell>
                    <TableCell className={classes.tableCell}>{betevo.betevo_home_total_odd+" "}{betevo.betevo_home_total_stand}</TableCell>
                    <TableCell className={classes.tableCell}>{betevo.sport_home_total_odd+" "}{betevo.sport_home_total_stand}</TableCell>
                  </TableRow>
                </TableBody>
              </Table>
            </CardContent>
