...
betevo_collection = db.mc_betevo88s
            ...
            for i in range(len(folder_array)-1):
                ...
                for j in range(len(time_array)-1):
                    ...
                    for k in range(start_time_row+1, end_time_row):
                        ...
                        total_info = ""
                        total_info_cell = team_data.find("div", {"class": "total-cell"}).find("span", {"class": "ng-value-label"})
                        if total_info_cell == None : 
                            total_info_cell = team_data.find("div", {"class", "total-cell"}).find("div", {"class", "straight-cell"})
                        if total_info_cell :
                            total_info = total_info_cell.text
                        spread_infos = get_spread_infos(spread_odd_info)
                        total_infos = get_spread_infos(total_info)
                        print(total_info_cell, total_infos)
                        if team_info:
                            current_date = datetime.datetime.now()
                            csv_data.append([checkbox_dic[item_id], team_info, spread_infos[0], spread_infos[1], money_line_info, total_infos[0], total_infos[1], game_id, game_time, game_name])
                            document = {
                                "check_id": checkbox_dic[item_id],
                                 "team_info": team_info,
                                 "spread_odd": spread_infos[0],
                                 "spread_stand": spread_infos[1],
                                 "money_line_info": money_line_info,
                                 "total_odd": total_infos[0],
                                 "total_stand": total_infos[1],
                                 "game_id": game_id,
                                 "game_time": game_time,
                                 "game_name": game_name,
                                 "game_datetime": current_date
                            }
                            result = betevo_collection.insert_one(document)
